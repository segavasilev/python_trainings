...
def timevalidator(time):
    ...
    if m is None :
        ...
        return True
mask = {
    "bus_id": {'type': int, 'required': 'required', 'format': '', 'routes':{128: ['Prospect Avenue', 'Elm Street', 'Sunset Bouleward', 'Seasame Street'],
    ...
    "stop_type": {'type': str, 'required': 'optional', 'format': 'UPPER'},
...
for i in range(len(read)):
    ...
    if isinstance(stop_id, mask['stop_id']['type']) == False:
        med['stop_id'] += 1
    if isinstance(stop_name, mask['stop_name']['type']) == False: #  or len([i for i in stop_name.split(' ') if i.istitle()]) != 2:
        med['stop_name'] += 1
    if isinstance(next_stop, mask['next_stop']['type']) == False:
        med['next_stop'] += 1
    if isinstance(stop_type, mask['stop_type']['type']) == False or (stop_type != '' and stop_type.isupper() == False):
        med['stop_type'] += 1
